  ...
  //oop
    var area = function(arr,popup){ // 輸入一系列area資料
      ...
      this.polygon.on('mouseover',()=>{
        console.log('bb');
        this.polygon.setStyle({color:'#00ffff'})
      })
      this.polygon.on('mouseout',()=>{
        console.log('ll');
        this.polygon.setStyle({color:'#ff00ff'})
      })
      // setting hover      
      ...
      if (p5j.mouseX>0 && p5j.mouseX<$('#map').width() && p5j.mouseY>0 && p5j.mouseY<$('#map').height() ){ 
        ...
        if (p5j.mouseButton === 'left'){
          ...
          if (p5j.arr.length < 3){
            ...
            p5j.arrarea.push(new area(p5j.arr,"Hi~"));
            p5j.arr = [];
    ...
    $('#lbutton').click(()=>{
        let url = 'https://spreadsheets.google.com/feeds/list/1y8e98C7U22eRyPQNYG9npFbHnrQK2cRQdvLnGeLpIKg/1/public/values?alt=json';
        p5j.loaded=0;
        //console.log(url);
        p5j.httpGet(url, 'jsonp', false, function(response) {
    ...
    $('#ubutton').click(()=>{
        if(p5j.arrarea.length>0){
          ...
          let exeurl = 'https://script.google.com/macros/s/AKfycbxydBl81fYB5rtSUQ_SV4wwyryvItw2-EIXzLltoE1zalJn5g4/exec';          
          let editurl = 'https://docs.google.com/spreadsheets/d/1y8e98C7U22eRyPQNYG9npFbHnrQK2cRQdvLnGeLpIKg/edit#gid=0';
